			...
			<main className='relative z-10 flex-1 px-6 pt-6 pb-24 space-y-6'>
				...
				{error && (
					<motion.div
						...
						className='p-6 bg-red-50/80 border border-red-200 rounded-[2rem] text-center space-y-2'>
						<AlertCircle className='w-10 h-10 text-red-500 mx-auto' />
						<p className='text-red-800 font-bold'>डेटा लोड करने में त्रुटि</p>
						<p className='text-xs text-red-600'>{error}</p>
						<button
							onClick={fetchDashboard}
							className='text-xs bg-white py-2 px-4 rounded-xl font-bold shadow-sm mt-2'>
							पुनः प्रयास करें
						</button>
					</motion.div>
				)}
						...
						<motion.div
							...
							<div className='relative overflow-hidden p-6 rounded-[2.5rem] bg-slate-900 text-white shadow-2xl shadow-slate-300'>
								...
								<div className='relative z-10 flex flex-col items-center text-center space-y-1'>
									<p className='opacity-70 text-[10px] font-black uppercase tracking-[0.2em]'>
										उपलब्ध नकद (Cash in Hand)
									</p>
...
function StatCard({ label, sub, amount, icon: Icon, theme, delay }) {
	...
	return (
		<motion.div
			...
			<div
				className={`w-10 h-10 rounded-2xl flex items-center justify-center ${colors[theme]} border mb-2`}>
				<Icon size={20} />
			</div>
			<div>
				...
				<p className='text-xs font-bold text-slate-600 leading-none mt-1'>
					{label}
				</p>
	...
	return (
		<motion.div
			...
			<div className='flex items-center gap-4'>
				<div
					className={`w-10 h-10 rounded-2xl flex items-center justify-center ${colors[theme]} border`}>
					<Icon size={20} />
				</div>
