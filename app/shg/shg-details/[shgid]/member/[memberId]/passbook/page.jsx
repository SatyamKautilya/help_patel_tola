...
import { ChevronLeft, Wallet, TrendingDown, Landmark, ArrowRight } from "lucide-react";
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800">सदस्य पासबुक सारांश</h1>
              <p className="text-xs text-slate-500">सदस्य स्तर की वित्तीय जानकारी</p>
            </div>
          </div>
          <button
            onClick={() => router.back()}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-bold text-slate-800">SHG सारांश</h2>
                {shgSummary?.lastUpdated ? (
                  <p className="text-[11px] text-slate-500">
                    अपडेट: {new Date(shgSummary.lastUpdated).toLocaleDateString("hi-IN", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })}
              ...
              {shgSummaryLoading ? (
                <p className="text-sm text-slate-500">SHG सारांश लोड हो रहा है...</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <SummaryRow label="उपलब्ध नकद" value={formatMoney(shgSummary?.totalAvailableCash)} />
                  <SummaryRow label="मासिक बचत" value={formatMoney(shgSummary?.totalMonthlySavings)} />
                  <SummaryRow label="लंप सम राशि" value={formatMoney(shgSummary?.totalLumpSum)} />
                  <SummaryRow label="ब्याज संग्रह" value={formatMoney(shgSummary?.totalInterestCollected)} />
                  <SummaryRow label="पेनल्टी" value={formatMoney(shgSummary?.totalPenalty)} />
                  <SummaryRow label="ऋण वितरण" value={formatMoney(shgSummary?.totalLoanGiven)} />
                  <SummaryRow label="कुल खर्च" value={formatMoney(shgSummary?.totalExpense)} />
                </div>
              )}
            </motion.section>
          </>
...
function StatCard({ icon, label, value, tint, valueClass }) {
  return (
    <div className={`rounded-xl border p-4 ${tint}`}>
      ...
      <p className={`mt-2 text-2xl font-black ${valueClass}`}>₹{Number(value || 0)}</p>
    </div>
  );
}
